...
function FloorPlanPage() {
  ...
  // Mobile View Navigation Tab State
  const [mobileTab, setMobileTab] = useState<"canvas" | "planner" | "inspector" | "saved">("canvas");
  ...
  const exportSvg = () => {
    ...
    a.href = url;
    a.download = `${(planName || "floor-plan").replace(/[^a-zA-Z0-9-]/g, "-").toLowerCase()}.svg`;
    a.click();
    URL.revokeObjectURL(url);
    toast.success("Floor Plan exported as SVG");
  };

  // Dragging event handlers on canvas with touch-action safety
  ...
  // 1. Space Planner Sidebar Panel (Plot size, built-up size, BHK, facing, instructions, templates)
  const renderSpacePlannerPanel = () => (
    <aside className="surface-panel h-fit space-y-4 p-3.5 sm:p-4">
      <div className="space-y-3.5">
        <div className="flex items-center justify-between">
          <p className="label-caps flex items-center gap-1.5 text-xs font-bold">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <p className="label-caps text-xs">Curated Templates</p>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <Button size="sm" variant="ghost" onClick={addRoom} className="text-xs mt-1 w-full justify-center">
  ...
  // 2. Room Inspector Panel
  const renderInspectorPanel = () => (
    <div className="surface-panel h-fit space-y-4 p-3.5 sm:p-4">
      <div className="flex items-center justify-between">
        <p className="label-caps text-xs font-bold flex items-center gap-1.5">
          <PenTool className="size-3.5 text-primary" /> Room Inspector
        </p>
        {selectedRoom && (
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            <Label className="text-xs flex items-center gap-1">
              <Layers className="size-3 text-muted-foreground" /> Zone Type
            </Label>
            <Select
          ...
          <div className="flex items-center justify-between border-t border-border pt-3">
            ...
            <Button
              ...
              onClick={removeRoom}
            >
              <Trash2 className="size-3.5 mr-1" /> Delete Room
            </Button>
          </div>
        </div>
      ) : (
        <div className="p-4 text-center text-xs text-muted-foreground border border-dashed rounded-lg">
          Tap any room on the canvas to inspect, resize, and edit its attributes.
        </div>
      )}
    </div>
  );
  ...
  // 3. Saved Plans Panel
  const renderSavedPlansPanel = () => (
    <div className="surface-panel h-fit space-y-3 p-3.5 sm:p-4">
      <div className="flex items-center justify-between">
        <p className="label-caps text-xs font-bold flex items-center gap-1.5">
          <FolderOpen className="size-3.5 text-primary" /> Saved Floor Plans
        </p>
  ...
  // 4. Center Interactive 2D Presentation Canvas
  const renderCanvasSection = () => (
    <section className="surface-panel overflow-hidden p-3 sm:p-4 flex flex-col">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2.5">
        <div className="space-y-0.5 flex-1 min-w-[180px]">
          <Label htmlFor="planName" className="text-[11px] text-muted-foreground">
            House Plan Title
          </Label>
          <Input
            id="planName"
            ...
            className="h-8 font-bold text-xs sm:text-sm uppercase tracking-wide"
          />
        </div>
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <Badge variant="secondary" className="text-[11px] font-mono text-primary font-semibold whitespace-nowrap">
            Built-up: {totalArea} sq ft
          </Badge>
        </div>
      </div>

      <div className="flex-1 min-h-[380px] sm:min-h-[480px] max-h-[70vh] sm:max-h-[82vh] overflow-x-auto overflow-y-auto rounded-lg border border-border p-2 sm:p-4 relative bg-[#f1f5f9] dark:bg-[#0b1329] flex items-center justify-center">
        {/* Vastu Compass Indicator Badge (Mobile Compact) */}
        <div className="absolute top-2.5 right-2.5 sm:top-4 sm:right-4 z-10 backdrop-blur border rounded-md px-1.5 py-1 sm:p-2 shadow-sm pointer-events-none flex flex-col items-center bg-white/95 border-slate-300 text-slate-800 dark:bg-slate-900/95 dark:border-slate-700 dark:text-slate-200">
          <Compass className="size-3.5 sm:size-5 text-primary animate-pulse" />
          <span className="text-[8px] sm:text-[9px] font-bold tracking-widest mt-0.5">{facing.toUpperCase()} ↑</span>
        </div>
        ...
        <svg
          ...
          className="w-full max-w-2xl rounded-lg shadow-xl select-none mx-auto touch-manipulation"
          style={{
            maxHeight: "68vh",
            aspectRatio: `${maxX + 8} / ${maxY + 10}`,
      ...
      {/* Quick Mobile Selected Room Bar */}
      {selectedRoom && (
        <div className="lg:hidden mt-2 p-2 bg-secondary/50 rounded-md border border-border flex items-center justify-between gap-2 text-xs">
          <div className="truncate">
            <span className="font-semibold text-foreground">{selectedRoom.name}</span>
            <span className="text-muted-foreground ml-1.5 font-mono">
              {selectedRoom.w}′ × {selectedRoom.h}′ ({selectedRoom.w * selectedRoom.h} sq ft)
            </span>
          </div>
          <Button
            variant="outline"
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-border pt-2 text-[11px] sm:text-xs text-muted-foreground">
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          <span className="flex items-center gap-1 font-medium text-foreground">
            <CheckCircle2 className="size-3.5 text-emerald-500" /> 2D Colored Plan
          </span>
          <span>
          ...
          <span className="hidden sm:inline">Grid: 1 ft Snap</span>
        </div>
        <div className="flex items-center gap-2">
  ...
  return (
    <div className="space-y-4">
        ...
        actions={
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
            <div className="flex items-center gap-1 border border-border rounded-md p-0.5 bg-background">
              ...
              <span className="text-[10px] font-mono px-1 text-muted-foreground">{Math.round(zoom * 100)}%</span>
              <Button
                variant="ghost"
                size="sm"
                className="h-7 w-7 p-0"
            ...
            <Button variant="outline" size="sm" onClick={exportSvg} className="gap-1 px-2.5 sm:px-3 text-xs font-medium h-8">
            ...
            <Button
              variant="secondary"
              size="sm"
              className="gap-1 px-2.5 sm:px-3 text-xs border border-primary/30 text-primary hover:bg-primary/10 h-8"
              onClick={handleRender3D}
            >
              <Sparkles className="size-3.5 text-primary" />
              <span className="hidden sm:inline">Render </span>3D View
            </Button>
            <Button size="sm" onClick={handleSave} disabled={savePlan.isPending} className="gap-1 px-2.5 sm:px-3 text-xs h-8">
              {savePlan.isPending ? (
                <Loader2 className="size-3.5 animate-spin" />
              ) : (
                <Save className="size-3.5" />
              )}
              <span>{planId ? "Update" : "Save"}</span>
            </Button>
          </div>
        }
      />

      {/* --- MOBILE NAVIGATION SEGMENTED TABS (Visible only on mobile/tablet < lg) --- */}
      <div className="lg:hidden">
        <div className="grid grid-cols-4 gap-1 p-1 bg-secondary/70 rounded-lg border border-border text-xs font-medium">
          <button
            type="button"
      ...
      {/* --- DESKTOP 3-COLUMN LAYOUT / MOBILE ACTIVE TAB --- */}
      <div className="lg:grid lg:gap-6 lg:grid-cols-[300px_minmax(0,1fr)_300px]">
        {/* Column 1: Space Planner (Desktop visible, mobile conditional) */}
        <div className={`space-y-5 ${mobileTab === "planner" ? "block" : "hidden lg:block"}`}>
          {renderSpacePlannerPanel()}
        </div>

        {/* Column 2: Center Canvas (Desktop visible, mobile conditional) */}
        <div className={`${mobileTab === "canvas" ? "block" : "hidden lg:block"}`}>
          {renderCanvasSection()}
        </div>

        {/* Column 3: Room Inspector & Saved Plans (Desktop visible, mobile conditional) */}
        <aside className={`space-y-5 ${mobileTab === "inspector" || mobileTab === "saved" ? "block" : "hidden lg:block"}`}>
          {/* On mobile show Inspector or Saved based on active tab; on desktop show both */}
          <div className={`${mobileTab === "inspector" || mobileTab === "canvas" ? "block" : "hidden lg:block"}`}>
            {renderInspectorPanel()}
          </div>
          <div className={`${mobileTab === "saved" ? "block" : "hidden lg:block"}`}>
      ...
      <Dialog open={render3DOpen} onOpenChange={setRender3DOpen}>
        <DialogContent className="w-[95vw] sm:max-w-4xl max-h-[90vh] overflow-y-auto bg-background/95 backdrop-blur p-3.5 sm:p-6">
          <DialogTitle className="flex items-center gap-2 text-base sm:text-lg font-semibold">
            <Box className="size-4 sm:size-5 text-primary" /> 3D Isometric Architectural Cutaway
          </DialogTitle>
          ...
          <div className="space-y-3 pt-2">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 bg-secondary/30 p-2.5 sm:p-3 rounded-lg border border-border/60">
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                <div className="flex items-center gap-2">
                  <Label className="text-xs font-medium shrink-0">Style:</Label>
                ...
                <div className="flex items-center gap-2">
                  <Label className="text-xs font-medium shrink-0">Palette:</Label>
            ...
            <div className="relative aspect-[16/9] w-full overflow-hidden rounded-lg border border-border bg-secondary/30 grid place-items-center">
              {render3DResult ? (
                ...
                <div className="text-center space-y-2 p-4">
                  ...
                  <p className="text-xs sm:text-sm font-medium">Generating 3D Isometric Cutaway…</p>
                ...
                <div className="text-center space-y-1.5 p-4">
                  <Box className="mx-auto size-7 text-muted-foreground/60" />
            ...
            {render3DResult && (
              <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  >
                    <FileText className="size-3.5 mr-1" /> Sheet
                  </Button>
